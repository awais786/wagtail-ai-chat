...
class RAGChatBot:
    ...
    def __init__(
        ...
        self.llm_provider = llm_provider or getattr(
            settings, "WAGTAIL_RAG_LLM_PROVIDER", "ollama"
        )
        self.model_name = model_name or getattr(
            settings, "WAGTAIL_RAG_MODEL_NAME", None
        )
        ...
        use_hybrid_search_setting = getattr(
            settings, "WAGTAIL_RAG_USE_HYBRID_SEARCH", True
        )
        ...
        if use_hybrid_search_setting and not _is_wagtail_available():
            logger.warning(
                "WAGTAIL_RAG_USE_HYBRID_SEARCH is True but Wagtail is not available. Using vector-only search."
            )
        elif not use_hybrid_search_setting:
            logger.info(
                "WAGTAIL_RAG_USE_HYBRID_SEARCH is False. Using vector-only search (Wagtail search disabled)."
            )
    ...
    def _log_configuration(self, embedding_provider, embedding_model):
        ...
        search_mode = (
            "Hybrid (vector + Wagtail)"
            if use_hybrid
            else "Vector-only (Wagtail disabled)"
        )
    ...
    def _create_vectorstore(self):
        """Create and return the vectorstore (ChromaDB, FAISS, or pgvector)."""
        ...
        if backend == "chroma":
            ...
            if os.path.exists(index_path):
                try:
                    ...
                    logger.warning(
                        "Could not load existing FAISS index, creating new one: %s", e
                    )
    ...
    def _create_llm(self, llm_kwargs):
        ...
        llm = get_llm(
            provider=self.llm_provider, model_name=self.model_name, **llm_kwargs
        )
        logger.info(
            f"LLM initialized successfully: {self.llm_provider} / {self.model_name or 'default'}"
        )
        return llm
    ...
    def _create_retriever(self):
        ...
        if use_multi_query:
            logger.info(
                f"LLM Query Expansion enabled - using MultiQueryRetriever for query expansion"
            )
            try:
                ...
                logger.warning(
                    "Failed to initialize MultiQueryRetriever, falling back to base retriever"
                )
                pass
        else:
            if not MULTI_QUERY_AVAILABLE:
                logger.info(
                    "LLM Query Expansion disabled - MultiQueryRetriever not available"
                )
            else:
    ...
    def query(
        ...
        2. LLM generation: Generates an answer using the LLM with the retrieved context (unless search_only=True)
        ...
        Args:
            ...
            search_only: If True, skip LLM generation and return only semantic search results

        Returns:
            A dict with:
              - 'answer': string generated by the LLM (or None if search_only=True)
        ...
        logger.info(
            f"Generating answer using {self.llm_provider}/{self.model_name or 'default'} for question: '{question}'"
        )
        ...
        logger.info(
            f"LLM Result: {answer[:200] if isinstance(answer, str) else str(answer)[:200]}{'...' if (len(answer) if isinstance(answer, str) else len(str(answer))) > 200 else ''}"
        )
