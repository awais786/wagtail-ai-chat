...
class EmbeddingSearcher:
    ...
    def _get_vector_docs(self, query: str) -> Tuple[List[Document], Set[str], Set[Any]]:
        ...
        try:
            ...
            logger.debug(
                "retriever.invoke failed (%s), falling back to vectorstore.similarity_search()",
                e,
            )
            try:
    ...
    def _get_wagtail_docs(
        ...
        try:
            ...
            if normalized_query != query:
                logger.debug(
                    f"Normalized Wagtail query: '{query}' -> '{normalized_query}'"
                )
            ...
            if len(wagtail_results) == 0 and normalized_query != query:
                logger.debug(
                    f"Trying original query after normalized query found 0 results"
                )
            ...
            for page in wagtail_results:
                ...
                if page_docs:
                    ...
                    logger.debug(
                        f"Added {len(page_docs)} document(s) from page ID: {page_id}"
                    )
            ...
            logger.warning(
                f"Wagtail search failed for query '{query}': {e}", exc_info=True
            )
            ...
            for doc, score in all_results:
                ...
                if query_clean in title or title.startswith(query_clean):
                    ...
                    for char in title_chars:
                        if (
                            query_idx < len(query_chars)
                            and char == query_chars[query_idx]
                        ):
                            query_idx += 1
                    ...
                    for query_word in query_words:
                        if len(query_word) >= 4:
                            ...
                            for title_word in title_words:
                                if len(title_word) >= 4 and self._fuzzy_match(
                                    query_word, title_word
                                ):
                                    score = max(score, 0.7)
    ...
    def _apply_conservative_title_boost(
        ...
        max_title_boost_score = getattr(
            settings, "WAGTAIL_RAG_TITLE_BOOST_MAX_SCORE", None
        )
        ...
        for result in results:
            ...
            score_ok = (
                ...
                else result.get("score", float("inf")) <= max_title_boost_score
            )
