...
class Command(BaseCommand):
    ...
    def _display_config(self):
        ...
        use_multi_query = getattr(settings, 'WAGTAIL_RAG_USE_LLM_QUERY_EXPANSION', False)
    ...
    def _handle_interactive_mode(self, chatbot, session_id, no_history, no_sources):
        ...
        if not no_history and getattr(settings, 'WAGTAIL_RAG_ENABLE_CHAT_HISTORY', True):
            ...
            use_history = True
            self.stdout.write(self.style.SUCCESS(f'Chat session started: {session_id}\n'))
        else:
            use_history = False
        ...
        while True:
            try:
                ...
                # Display answer
                self.stdout.write(self.style.SUCCESS('Bot: ') + result.get('answer', 'No answer generated'))
                self.stdout.write('')
